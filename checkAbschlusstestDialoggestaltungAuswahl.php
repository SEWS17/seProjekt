...
$sql = "SELECT id, ATEXT, BILD1, Bildbeschreibung1, BILD2, Bildbeschreibung2, KORREKTE_ANTWORT FROM abschlusstest_auswahl_dialoggestaltung";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        if(isset($_GET["id"]) && $row["id"] == $_GET['id']) {
            ...
            echo "> <img src='".$row["BILD1"]."' alt ='".$row["Bildbeschreibung1"]."' height='200px' width='360px';></label>";

            echo "<label><input type='radio'style='display:none;' name='Kategorie' value='auswahl2'";
            if(@$_POST['Kategorie'] == 'auswahl2') {echo "checked";}
            echo "> <img src='".$row["BILD2"]."' alt ='".$row["Bildbeschreibung2"]."' height='200px' width='360px';></label>";
